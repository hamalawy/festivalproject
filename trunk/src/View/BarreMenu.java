...
public class BarreMenu extends JMenuBar{
	...
	private JMenuItem groupe_inscription, groupe_annulation, groupe_listing, groupe_recherche;
	private JMenuItem personnel_ajout, personnel_listing, personnel_horaire, personnel_recherche;
	private JMenuItem scene_enregCarac, scene_listing, scene_horaireConcerts;
	private JMenuItem carte;
	...
	public BarreMenu(MainFrame frame)
	{
		...
		groupe_recherche = new JMenuItem("Rechercher un groupe");
		...
		personnel_recherche = new JMenuItem("Rechercher un travailleur");
		...
		carte.setMnemonic(KeyEvent.VK_C);
		...
		groupe_recherche.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_R, InputEvent.ALT_MASK));
		groupe_recherche.setMnemonic(KeyEvent.VK_R);
				//Personnel
		...
		personnel_recherche.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_R, InputEvent.ALT_MASK));
		personnel_recherche.setMnemonic(KeyEvent.VK_R);
				//Scene
		...
		groupe_recherche.addActionListener(am);
		groupe.add(groupe_recherche);
		...
		personnel_recherche.addActionListener(am);
		personnel.add(personnel_recherche);
	...
	private class ActionManager implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(e.getSource() == stand_inscription)
				frame.afficherStandInscription();
			if(e.getSource() == stand_listing)
				frame.afficherStandListing();
			if(e.getSource() == stand_recherche)
				frame.afficherStandRecherche();
			if(e.getSource() == groupe_inscription)
				frame.afficherGroupeInscription();
			if(e.getSource() == groupe_annulation)
				frame.afficherGroupeAnnulation();
			if(e.getSource() == groupe_listing)	
				frame.afficherGroupeListing();
			if(e.getSource() == groupe_recherche)
				frame.afficherGroupeRecherche();
			if(e.getSource() == personnel_ajout)
				frame.afficherPersonnelAjout();
			if(e.getSource() == personnel_horaire)	
				frame.afficherPersonnelHoraire();
			if(e.getSource() == personnel_listing)		
				frame.afficherPersonnelListing();
			if(e.getSource() == personnel_recherche)	
				frame.afficherPersonnelRecherche();
		    if(e.getSource() == scene_enregCarac)	
		    	frame.afficherSceneEnregCarac();
			if(e.getSource() == scene_horaireConcerts)		
				frame.afficherSceneHoraireConcerts();
